...
class ASGDRO(ERM):
    ...
    """
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(ASGDRO, self).__init__(input_shape, num_classes, num_domains,
                                        hparams)

        self.register_buffer("q", torch.Tensor())
        self.container_optimizer = torch.optim.Adam
        self.optimizer = self.SAM(self.network.parameters(), self.container_optimizer, adaptive=True,
    ...
    def update(self, x, y, **kwargs):
        ...
        prev_losses = torch.zeros(len(minibatches)).to(device)
        for m in range(len(minibatches)):
            x,y = minibatches[m]
            prev_losses[m] = F.cross_entropy(self.predict(x),y)

        prev_losses.mean().backward()
    ...
    class SAM(torch.optim.Optimizer):
        """
        duplication from https://github.com/davda54/sam
        """
        def __init__(self, params, base_optimizer, rho=0.05, adaptive=False, **kwargs):
            assert rho >= 0.0, f"Invalid rho, should be non-negative: {rho}"

            defaults = dict(rho=rho, adaptive=adaptive, **kwargs)
            super(ASGDRO.SAM, self).__init__(params, defaults)
        ...
        def disable_running_stats(model):
            "from @ahmdtaha in https://github.com/davda54/sam"
            def _disable(module):
