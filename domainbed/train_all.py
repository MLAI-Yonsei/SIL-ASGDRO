...
def main():
    ...
    ##################
    parser.add_argument("--result_path", type=str, default="neurips_rebut")
    ...
    if type(args.test_envs[0]) == int:
        args.test_envs = [args.test_envs]
    for idxx, test_env in enumerate(args.test_envs):
        res, records = train(
            test_env,
            args=args,
            hparams=hparams,
    ...
    ## Result Merging
    merged = dict()
    acc_result = results["iid"]
    oracle_result = results["oracle"]
    
    for i in range(len(acc_result)):
        merged[f"env_{i}"] = acc_result[i]
    merged["avg"] = np.mean(acc_result)
    merged["std"] = np.std(acc_result)
    merged["lr"] = hparams["lr"]
    merged["wd"] = hparams["weight_decay"]
    merged["bs"] = hparams["batch_size"]
    
    if args.algorithm == "ASGDRO":
        merged["rb"] = hparams["groupdro_eta"]
        merged["rho"] = hparams["rho"]
    
    for i in range(len(acc_result)):
        merged[f"oracle/env_{i}"] = oracle_result[i]
    merged["oracle/avg"] = np.mean(oracle_result)    

    if args.add_name:
        name += args.add_name
        
    merged["name"] = name
    merged["time"] = end_time - start_time
    merged_df = pd.DataFrame([merged])
    
    if os.path.exists(path_):
        dff = pd.read_csv(path_)
        dff = pd.concat([merged_df,dff])
        dff.to_csv(path_, index=False)
    else:
