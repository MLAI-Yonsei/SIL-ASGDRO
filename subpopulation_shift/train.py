...
def run_epoch(epoch, model, optimizer, loader, loss_computer, logger, csv_logger, args,
              is_training, show_progress=False, log_every=50, scheduler=None, sam_loss_computer=None):
    ...
    with torch.set_grad_enabled(is_training):
        ...
        for batch_idx, batch in enumerate(prog_bar_loader):
            ...
            if sam_loss_computer:
                loss_main = loss_computer.loss(outputs, y, g, is_training, mode="first_step")
            else:
            ...
            if is_training:
                if sam_loss_computer:
                        ...
                        loss_main = sam_loss_computer.loss(model(x), y, g, is_training,
                                                           mode="second_step", prev_stat=loss_computer.group_acc)
                        loss_main.backward()
                        if 'bert' in args.model:
                            torch.nn.utils.clip_grad_norm_(model.parameters(), args.max_grad_norm)
                            optimizer.seoncd_step(zero_grad=True)
                            scheduler.step()
                        else:
            ...
            if is_training and (batch_idx+1) % log_every==0:
                if sam_loss_computer:
                    csv_logger.log(epoch, batch_idx, sam_loss_computer.get_stats(model, args))
                    csv_logger.flush()
                    sam_loss_computer.log_stats(logger, is_training)
                    sam_loss_computer.reset_stats()
...
def train(model, criterion, dataset,
    ...
    train_loss_computer = LossComputer(
        args,
        criterion,
        is_robust=False if args.asam or args.asgdro else args.robust,
    ...
    if args.asam:#TODO:
        sam_loss_computer = LossComputer(
                args,
                criterion,
                is_robust=False,
                dataset=dataset["train_data"],
    ...
    if 'bert' in args.model:
        ...
        if args.scheduler == 'ReduceLROnPlateau':
            scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
                optimizer if not sam_loss_computer else optimizer.base_optimizer,
                'min',
                factor=0.1,
                patience=5,
                threshold=0.0001,
            ...
            scheduler = StepLR(optimizer if not sam_loss_computer else optimizer.base_optimizer,
                               t_total,
    ...
    for epoch in range(epoch_offset, epoch_offset+args.n_epochs):
        if args.lisa_mix_up:
            ...
            run_epoch(
                ...
                log_every=args.log_every,
                scheduler=scheduler,
                sam_loss_computer=sam_loss_computer)
