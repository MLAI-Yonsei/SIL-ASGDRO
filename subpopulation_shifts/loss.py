...
class LossComputer:
    ...
    def loss(self, yhat, y, group_idx=None, is_training=False, mix_up=False, y_onehot=None, return_group_loss=False,
        ...
        if (not mix_up) or (mode != 'first_step'):
    ...
    def log_stats(self, logger, is_training):
        ...
        for group_idx in range(self.n_groups):
            group_str = self.group_str(group_idx, is_training or self.is_val) if 'Meta' in self.args.dataset else self.group_str(group_idx)
            logger.write(
                f'  {group_str}  '
