...
def main():
    ...
    parser.add_argument("--asgdro", default=False, action='store_true')
    parser.add_argument("--asam", default=False, action='store_true')
    ...
    if resume:
        ...
        if args.is_featurizer:
            if args.model == 'bert':
                ...
                from bert.distilbert import IBertFeaturizer
                featurizer = IBertFeaturizer.from_pretrained("kssteven/ibert-roberta-base", **args.model_kwargs)
                classifier = nn.Linear(featurizer.d_out, 5 if args.dataset == "Amazon" else n_classes)
                model = torch.nn.Sequential(featurizer, classifier)
            else:
                raise NotImplementedError
